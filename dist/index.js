...
import { z } from "zod";
import { searchProjects, resolveProjectIdToken, fetchProjectDetail, RajReraError, } from "./client.js";
...
server.registerTool("get_project_details", {
    ...
    description: "Fetch the latest registered details for a Rajasthan RERA project by name. " +
    ...
    inputSchema: {
        ...
        registrationNo: z
            .string()
            .optional()
            .describe("Optional RERA registration number (e.g. 'RAJ/P/2024/3092') to pick " +
    ...
    try {
        ...
        if (results.length === 0) {
            return text(`No projects found on the Rajasthan RERA portal matching "${projectName}".`);
        }
        // Narrow by registration number if provided.
        let candidates = results;
        if (registrationNo) {
            const wanted = registrationNo.trim().toLowerCase();
            candidates = results.filter((r) => r.registrationNo.toLowerCase() === wanted);
            if (candidates.length === 0) {
                return text(`No project named like "${projectName}" has registration number ` +
                    `"${registrationNo}". Candidates found:\n\n${formatCandidates(results)}`);
            }
        }
        if (candidates.length > 1) {
            return text(`Multiple projects match "${projectName}". Re-run get_project_details ` +
        ...
        const msg = err instanceof RajReraError
            ? err.message
...
function formatCandidates(items) {
    return items
        .map((r, i) => `${i + 1}. ${r.projectName} — ${r.registrationNo}\n` +
