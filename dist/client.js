...
export async function searchProjects(projectName) {
    ...
    if (!res.ok) {
        throw new RajReraError(`Search failed: ${res.status} ${res.statusText}`);
    }
    let json;
    try {
        json = await res.json();
...
function normalizeSearchItem(d) {
    return {
        id: d.Id,
...
export async function resolveProjectIdToken(encryptedProjectId) {
    ...
    if (!res.ok) {
        throw new RajReraError(`ProjectDtls lookup failed: ${res.status} ${res.statusText}`);
    }
    let json;
    try {
        json = await res.json();
    ...
    const token = json?.data?.ProjectId;
    if (!token) {
        throw new RajReraError("ProjectDtls response did not contain data.ProjectId");
    }
    return token;
}
/** Step 3: fetch the full "updated" project record. */
export async function fetchProjectDetail(projectIdToken) {
    ...
    if (!res.ok) {
        throw new RajReraError(`ViewProject failed: ${res.status} ${res.statusText}`);
    }
    try {
...
function mapProjectStatus(code) {
    switch (code) {
        case 3:
            return "Registered";
        case 2:
